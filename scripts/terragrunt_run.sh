...
usage() {
    ...
    echo ""
    echo "環境変数:"
    echo "  target: 対象モジュール (source, iam, network, log, servise_staticweb, servise_dynamicweb, all) - 未指定時は 'all'"
    echo ""
    echo "例:"
    echo "  $0 -e dev -a plan"
    echo "  $0 -e dev -a apply -y"
    echo "  target=source $0 -e dev -a plan"
    echo "  target=iam $0 -e dev -a plan"
    echo "  target=network $0 -e dev -a plan"
    echo "  target=log $0 -e dev -a plan"
    echo "  target=servise_staticweb $0 -e dev -a plan"
    echo "  target=servise_dynamicweb $0 -e dev -a plan"
...
# ターゲットの検証
if [[ ! "$TARGET" =~ ^(source|iam|network|log|servise_staticweb|servise_dynamicweb|all)$ ]]; then
    echo "エラー: ターゲットは 'source', 'iam', 'network', 'log', 'servise_staticweb', 'servise_dynamicweb', または 'all' を指定してください"
    exit 1
fi
